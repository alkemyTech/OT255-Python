...
def manip_pandas_villa_maria():
    university = "villa_maria"
    # Path de los csv
    path_univ_csv = (
    ...
    df_univ["postal_code"] = df_univ["postal_code"].str.replace("[", " ")
    df_univ["postal_code"] = df_univ["postal_code"].str.replace("]", " ")
    df_univ = df_univ.drop("id", axis=1)
    ...
    # Cambio de formato para inscription_date: formato inicial: str -> cambio a datetime -> cambio a str
    df_univ["inscription_date"] = pd.to_datetime(
        df_univ["inscription_date"]
    ).dt.strftime("%Y-%m-%d")
    df_univ = df_univ.astype({"inscription_date": str})
