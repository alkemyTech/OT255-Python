def csvByLocation_to_txt_dag(university):
    from pathlib import Path

    import pandas as pd
    import sqlalchemy as db

    csvByLocation_to_txt(university)


def csvByPostalCode_to_txt_dag(university):
    from pathlib import Path

    import pandas as pd
    import sqlalchemy as db

    csvByPostalCode_to_txt(university)
...
def normalize_data_univ(university):
    ...
    # file = 'db_{}.csv'.load(university)
    # f_csv_path = Path.cwd() / "files" / "raw" / file
...
def csvByPostalCode_to_txt(university):
    ...
    postal_code_merge_db(university).to_csv(
        Path.cwd() / "files" / "modified" / ("G255_" + university + ".txt")
    )
