...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("univ_b.log"), logging.StreamHandler(sys.stdout)],
)
...
with airflow.DAG(
    ...
    extract_sql = python_operator.PythonOperator(
        task_id="extract_sql", python_callable=postgres
    )
    ...
    pandas_processing = python_operator.PythonOperator(
        task_id="pandas", python_callable=pandas_processing()
    )
